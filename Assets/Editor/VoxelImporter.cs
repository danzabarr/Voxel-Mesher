...
public class VoxelImporter : ScriptedImporter
{
    ...
    public static List<Volume> GreedyVolumes(Dictionary<Vector3Int, int> voxels)
    {
        
        Dictionary<Vector3Int, int> voxelsCopy = new Dictionary<Vector3Int, int>(voxels);
        // Greedily merge voxels into volumes
        List<Volume> volumes = new List<Volume>();
        void ExpandAlongAxis(Vector3Int p, ref Vector3Int size, int axis)
        {
            bool canExpand = true;
                ...
                for (int z = p.z; z < p.z + nextSize.z; z++)
                {
                    for (int y = p.y; y < p.y + nextSize.y; y++)
                    {
                        for (int x = p.x; x < p.x + nextSize.x; x++)
                        {
                            ...
                            if (!voxelsCopy.ContainsKey(checkPos) || voxelsCopy[checkPos] <= 0)
                            {
                                canExpand = false;
                                break;
                            }
        ...
        Vector3Int PopMin()
        {
            ...
            foreach (Vector3Int key in voxelsCopy.Keys)
                if (key.z < min.z || (key.z == min.z && (key.y < min.y || (key.y == min.y && key.x < min.x))))
                    min = key;
            return min;
        }

        while (voxelsCopy.Count > 0)
        {
            Vector3Int p = PopMin();
            if (voxelsCopy[p] <= 0)
                continue;
            ...
            ExpandAlongAxis(p, ref size, 1); // Expand along y-axis
            ExpandAlongAxis(p, ref size, 2); // Expand along z-axis
            ExpandAlongAxis(p, ref size, 0); // Expand along x-axis

            // remove the volume from the copy
            for (int z = p.z; z < p.z + size.z; z++)
                for (int y = p.y; y < p.y + size.y; y++)
                    for (int x = p.x; x < p.x + size.x; x++)
                        voxelsCopy.Remove(new Vector3Int(x, y, z));
...
/*
        ...
        foreach (KeyValuePair<Vector3Int, int> voxel in voxels)
        {
            ...
            if (voxel.Value <= 0 || visited.Contains(p))
                continue;
            ...
            // Mark the entire volume as visited
            for (int z = p.z; z < p.z + size.z; z++)
                for (int y = p.y; y < p.y + size.y; y++)
                    for (int x = p.x; x < p.x + size.x; x++)
    ...
    public override void OnImportAsset(AssetImportContext ctx)
    {
        ...
        // Voxel Volume
        VoxelMap map = new VoxelMap(data.voxels);
        

        // Mesh
        //Mesh mesh = volume.ConstructMesh(null, meshIndexFormat);
        ...
        * Matrix4x4.Translate(new Vector3(-anchor.x * data.size.x, -anchor.y * data.size.y, -anchor.z * data.size.z));
        Mesh mesh = Voxel.Mesh(map, Transform, meshIndexFormat);
        ...
        {    
            ...
            volume.Initialise(data, anchor, position, rotation, scale);   
        ...
        switch (colliderType)
        {
            ...
            {
                List<Volume> volumes = GreedyVolumes(map);
